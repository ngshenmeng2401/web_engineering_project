...
if (isset($_POST['remove_favourite'])){
    ...
    if(mysqli_query($conn, $deleteFavourite)){
        ...
        echo "<script type='text/javascript'>alert('Success!');window.location.assign('favourite_list.php');</script>'";
        
    }else{
        
        echo "<script type='text/javascript'>alert('Failed!!');window.location.assign('favourite_list.php');</script>'";
...
<head>
<meta charset="utf-8">
<title>favourite_list</title>
<link rel="stylesheet" href="../css/favourite_list.css">
	...
	<div class="body">
	    <?php
            ...
            if (isset($_POST['sort_uni_ranking_year'])){
                ...
                $sqlloadfavouriteuni = "SELECT * FROM tbl_ranking INNER JOIN tbl_favourite ON tbl_favourite.university_no = tbl_ranking.university_no WHERE year = '$year' AND user_email = '$email'";
                // $sqlloadfavouriteuni = "SELECT * FROM tbl_favourite WHERE year = '$year'";
                
            }else{
                $sqlloadfavouriteuni = "SELECT * FROM tbl_ranking INNER JOIN tbl_favourite ON tbl_favourite.university_no = tbl_ranking.university_no WHERE year = '2021' AND user_email = '$email'";
                // $sqlloadfavouriteuni = "SELECT * FROM tbl_favourite WHERE year = '2021'";
	    ...
	    <table id="example">
		    ...
		    <?php
            $result = $conn->query($sqlloadfavouriteuni);
		        if ($result->num_rows > 0){
		            while ($row = $result -> fetch_assoc()){
		                extract($row);
		                ...
		                <tr>
		                    ...
		                    <td style="text-align: center">
                                <form method="post" >
                                    ...
                                    <button type="submit" class="favourite_button favourite_button_color" name='remove_favourite'><i class="fa fa-heart" ></i></button>
                        		</form>
		                    </td>
		                </tr>
		            <?php
